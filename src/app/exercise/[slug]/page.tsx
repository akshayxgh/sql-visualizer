        ...
        <aside className="border-r border-neutral-800 flex flex-col overflow-y-auto scrollbar-thin">
          <div className="p-5 border-b border-neutral-800">
            <div className="flex items-center gap-2 mb-3 flex-wrap">
              ...
              />
              {exercise.topics.map((topic) => (                <Badge key={topic} label={topic} variant="topic" />              ))}
            </div>
          ...
          <div className="p-5 border-b border-neutral-800">
            ...
            <ul className="flex flex-col gap-2">
              {exercise.learningObjectives.map((obj, i) => (                <li key={i} className="flex items-start gap-2">                  <span className="text-brand-400 text-xs mt-0.5 flex-shrink-0">·</span>                  <span className="text-xs text-neutral-400 leading-relaxed">{obj}</span>                </li>              ))}
            </ul>
          </div>
            ...
            <div className="rounded-md border border-neutral-800 overflow-hidden">
              <div className="bg-neutral-900 px-3 py-2 border-b border-neutral-800">
                <span className="text-xs font-mono text-brand-400">
                  {dataset.displayName.toLowerCase()}                </span>
              </div>
              {dataset.columns.map((col) => (                <div
                  key={col.name}
                  className="flex items-center justify-between px-3 py-2 border-b border-neutral-800/60 last:border-0"
                >
                  <div className="flex items-center gap-2">
                    {col.isPrimaryKey && (                      <span className="text-amber-500 text-xs" title="Primary key">                        🔑                      </span>                    )}                    <span className="text-xs font-mono text-neutral-200">
                      {col.name}                    </span>
                  </div>
